...
def fisher_matrix_diag_nlp(t,train_data_loader,model,criterion,sbatch=20, opt = None):
    # Init
    fisher={}
    for n,p in model.named_parameters():
        fisher[n]=0*p.data
    ...
    for i_batch, sample_batched in enumerate(train_data_loader):
        ...
        # Forward and backward
        model.zero_grad()
        outputs,_ = model.forward(task, inputs)

        #loss = criterion(t, outputs[t], target) CODE ORIGINAL
    ...
    # Mean
    for n,_ in model.named_parameters():
        fisher[n]=fisher[n]/train_data_loader.__len__( )
        fisher[n]=torch.autograd.Variable(fisher[n],requires_grad=False)
    return fisher

    # for i in tqdm(range(0,train_data_loader.__len__( ),sbatch),desc='Fisher diagonal',ncols=100,ascii=True):
    #
    #
    #     b= torch.LongTensor(np.arange(i,np.min([i+sbatch,train_data_loader.__len__( )]))).cuda()
    #
    #
    #     images=torch.autograd.Variable(x[b],volatile=False)
    #     target=torch.autograd.Variable(y[b],volatile=False)
